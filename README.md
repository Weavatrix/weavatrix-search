...
harness, which compares an identical sorted match list against `fdir` plus
`fs.readFileSync` plus per-line JavaScript testing — what a Node consumer would
otherwise write, not ripgrep. Medians of three independent runs on a 15.9 MB
corpus: a cold search **4.66x** faster on Node and **4.75x** on Bun, and a
repeated query through a persistent index **253.8x** and **233.1x** faster.
